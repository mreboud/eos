    ...
    public:
        ...
        virtual void run() const
        {
...

            {
                ...
                TEST_CHECK_NEARLY_EQUAL( amp.omnes_factor(0.700332 , 0 , Isospin::zero).real()   ,  0.0646509 ,  eps);
                TEST_CHECK_NEARLY_EQUAL( amp.omnes_factor(0.700332 , 0 , Isospin::zero).imag()   ,  1.2225023 ,  eps);
                TEST_CHECK_NEARLY_EQUAL( amp.omnes_factor(25.0     , 0 , Isospin::zero).real()   , -0.0375782 ,  eps);
                TEST_CHECK_NEARLY_EQUAL( amp.omnes_factor(25.0     , 0 , Isospin::zero).imag()   ,  0.0017212 ,  eps);

                TEST_CHECK_NEARLY_EQUAL( amp.omnes_factor(0.700332 , 1 , Isospin::one).real()   , -2.5211430 ,  eps);
                TEST_CHECK_NEARLY_EQUAL( amp.omnes_factor(0.700332 , 1 , Isospin::one).imag()   ,  3.1724329 ,  eps);
                TEST_CHECK_NEARLY_EQUAL( amp.omnes_factor(25.0     , 1 , Isospin::one).real()   , -0.0277992 ,  eps);
                TEST_CHECK_NEARLY_EQUAL( amp.omnes_factor(25.0     , 1 , Isospin::one).imag()   ,  0.0010054 ,  eps);
